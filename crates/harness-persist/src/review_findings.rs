//! Persisted per-finding triage state for a `review-area` run.
//!
//! When a user acts on a finding in a run's review report — "Build this",
//! "Create issue", or "Ignore" — we remember it (keyed by the review run + a
//! stable finding key) so the report shows the same checkmarks / dimmed rows
//! next time it's opened. State is scoped to the run that produced the findings;
//! a fresh review run starts clean. Mirror of [`crate::geo_findings`].
...
const CREATE_REVIEW_FINDING_STATE: &str = "
CREATE TABLE IF NOT EXISTS harness_review_finding_state (
...
/// One finding's remembered triage state (matches `harness_review_finding_state`).
#[derive(Debug, Clone, Serialize, sqlx::FromRow)]
pub struct ReviewFindingState {
    /// The review run whose report this finding belongs to.
    pub run_id: String,
...
pub struct ReviewFindingStateInput {
    pub action: String,
    pub ref_run_id: Option<String>,
    pub issue_identifier: Option<String>,
    pub issue_url: Option<String>,
...
/// Postgres-backed store for review finding triage state.
pub struct ReviewFindingStateStore {
    pool: PgPool,
}

impl ReviewFindingStateStore {
    ...
    pub async fn from_pool(pool: PgPool) -> Result<Self, PersistError> {
        sqlx::query(CREATE_REVIEW_FINDING_STATE)
            .execute(&pool)
            .await?;
        Ok(Self { pool })
    }

    /// All remembered finding states for a review run.
    pub async fn list_for_run(
        &self,
        run_id: &str,
    ) -> Result<Vec<ReviewFindingState>, PersistError> {
        let rows = sqlx::query_as::<_, ReviewFindingState>(
            "SELECT run_id, finding_key, action, ref_run_id, issue_identifier, issue_url,
                    created_at, updated_at
             FROM harness_review_finding_state
             WHERE run_id = $1
    ...
    pub async fn set(
        ...
        run_id: &str,
        finding_key: &str,
        input: &ReviewFindingStateInput,
    ) -> Result<ReviewFindingState, PersistError> {
        let row = sqlx::query_as::<_, ReviewFindingState>(
            "INSERT INTO harness_review_finding_state (
    ...
    pub async fn clear(&self, run_id: &str, finding_key: &str) -> Result<bool, PersistError> {
        let result = sqlx::query(
            "DELETE FROM harness_review_finding_state WHERE run_id = $1 AND finding_key = $2",
        )
        .bind(run_id)
        .bind(finding_key)
        .execute(&self.pool)
...
mod tests {
    ...
    async fn round_trip_set_list_clear() {
        ...
        let store = ReviewFindingStateStore::connect(&url)
            .await
            .expect("connect");
        let run_id = format!(
            "review-run-{}",
        ...
        // Upsert to a different action keeps created_at.
        let updated = store
            .set(
                &run_id,
                key,
                &ReviewFindingStateInput {
                    action: "built".into(),
