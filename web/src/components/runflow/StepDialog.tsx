...
import { elapsedMs, formatDuration, formatTokens, statusLabel, totalTokens } from "./format";

const STATUS_VARIANT: Record<string, "running" | "success" | "failed" | "skipped" | "default"> = {
  running: "running",
  success: "success",
  failed: "failed",
  cancelled: "failed",
...
export function StepDialog({ view, onClose }: { view: NodeView | null; onClose: () => void }) {
  const now = Date.now();
  return (
        ...
        {view && (
          <>
            <DialogHeader>
              ...
              <DialogDescription className="sr-only">Step output and details</DialogDescription>
            </DialogHeader>
            ...
            <dl className="grid grid-cols-2 gap-x-6 gap-y-1.5 border-y py-3 text-xs sm:grid-cols-3">
              ...
              <Meta
                label="Duration"
                value={formatDuration(elapsedMs(view.started_at, view.ended_at, now))}
              />
              <Meta label="Iterations" value={String(view.iterations)} />
              <Meta label="Input tokens" value={formatTokens(view.usage.input)} />
              <Meta label="Output tokens" value={formatTokens(view.usage.output)} />
              {view.usage.cache_read != null && (
                <Meta label="Cache read" value={formatTokens(view.usage.cache_read)} />
              )}
              <Meta label="Total tokens" value={formatTokens(totalTokens(view.usage))} />
            </dl>

            {view.note && (
              <p className="border-b py-2 text-xs text-muted-foreground">{view.note}</p>
            )}
