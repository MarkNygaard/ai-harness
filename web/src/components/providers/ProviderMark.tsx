...
/**
 * One mark each. `viewBox` and `fillRule` default to the Simple Icons
 * convention, and are stated only where a brand does not follow it.
 */
interface Mark {
  d: string;
  viewBox?: string;
...
const PATHS: Record<string, Mark> = {
  claude: {
  ...
  // pi.dev's own mark: a blocky P with its counter knocked out, plus the dot of
  // the i. The 800-unit box and even-odd fill are the source's, kept rather
  // than re-drawn so this stays the mark instead of an approximation of one.
  pi: {
      ...
      <span className={TINT[provider] ?? "text-foreground"}>
        {mark ? (
          <svg
            ...
            fill="currentColor"
          >
            <path d={mark.d} fillRule={mark.fillRule} />
          </svg>
        ) : provider === "codex" ? (
