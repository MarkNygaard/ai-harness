...
describe("buildProjectSummaries", () => {
  it("folds cancelled into failed, sums completed into total, and sorts by total desc", () => {
    const rows: RunDailyCount[] = [
      { project: "proj-a", day: "2026-06-08T00:00:00Z", status: "completed", count: 3 },
      { project: "proj-a", day: "2026-06-08T00:00:00Z", status: "failed", count: 1 },
      { project: "proj-b", day: "2026-06-08T00:00:00Z", status: "completed", count: 1 },
      { project: "proj-b", day: "2026-06-08T00:00:00Z", status: "cancelled", count: 2 },
    ];
    const summaries = buildProjectSummaries(rows);
    expect(summaries).toHaveLength(2);
    expect(summaries[0].project).toBe("proj-a");
    expect(summaries[0].total).toBe(3);
    expect(summaries[0].byDay["2026-06-08"]).toEqual({ completed: 3, failed: 1 });
    expect(summaries[1].project).toBe("proj-b");
    expect(summaries[1].total).toBe(1);
    expect(summaries[1].byDay["2026-06-08"]).toEqual({ completed: 1, failed: 2 });
  });

  it("maps null/empty project to NO_PROJECT", () => {
    const rows: RunDailyCount[] = [
      { project: null, day: "2026-06-08T00:00:00Z", status: "completed", count: 1 },
      { project: "  ", day: "2026-06-08T00:00:00Z", status: "completed", count: 2 },
    ];
    const summaries = buildProjectSummaries(rows);
  ...
  it("falls back to project name sort when totals are tied", () => {
    const rows: RunDailyCount[] = [
      { project: "zebra", day: "2026-06-08T00:00:00Z", status: "completed", count: 1 },
      { project: "alpha", day: "2026-06-08T00:00:00Z", status: "completed", count: 1 },
    ];
    const summaries = buildProjectSummaries(rows);
