...
**Subscriptions page** under *Integrations*, not per project. The identity being
...
3. Click **Connect as app**. Linear asks a workspace admin to authorize —
   ...
   Subscriptions page with the result.
...
before, connect the workspace once on the Subscriptions page. (Per-project *GitHub*
...
| `412 {"error":"no Linear webhook signing secret configured"}` | ✅ Reached the harness. Save the signing secret on the Subscriptions page. |
...
| The project's Linear trigger button disappeared | The bindings dialog only appears once Linear is connected. Connect on the Subscriptions page. |
