...
description: "The YAML format for workflow DAGs: node types, when: gating, $node.output wiring, and the practices that keep a workflow correct against a real repo."
---
...
- **Pi nodes** (`pi`) — hooks become the `harness-hooks` Pi extension loaded
